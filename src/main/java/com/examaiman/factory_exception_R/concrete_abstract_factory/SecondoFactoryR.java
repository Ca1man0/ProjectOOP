...
// ORDINE 3 (FACTORY PATTERN) - RIF CONCRETE FACTORY P2-2 - SECONDOFACTORY.JAVA
// ORDINE 1 (EXCEPTION PATTERN) - RIF SERVICE e EXCEPTION SHIELDING PATTERN P2-2 - SECONDOFACTORY.JAVA

// Qui sono riportati gli stessi commenti di PrimoFactory...
 ...
 * l'oggetto specifico, ovveril piatto Secondo
 */

/**
 * EXCEPTION SHIELDING PATTERN (SERVICE e EXCEPTION SHIELDING PATTERN):
...
public class SecondoFactoryR extends PiattoFactoryR {
    private static final Logger logger = Logger.getLogger(SecondoFactoryR.class.getName());

    /**
     * RIF PATTERN FACTORY (CONCRETE CREATOR):
     * Qui sto implementando il metodo per creare l'oggetto specifico, piatto Secondo, specificando il tipo di piatto e gli ingredienti
    ...
    public PiattoR creaPiatto(String tipo, List<String> ingredienti) {
        ...
        /**
         * RIF EXCEPTION SHIELDING PATTERN (SERVICE):
        ...
        return switch (tipo.toLowerCase()) { // lowercase serve per gestire maiuscole, rendendo omogeneo l'input
            case "carne" -> new CarneR(ingredienti); // creo un nuovo oggetto che si basa sulla classe Carne e lo inizializzo con ingredienti

            // Se il valore non corrisponde ai case citat in precedenza, allora lancia l'eccezione
            default -> throw new UnsupportedOperationException("Tipo di secondo piatto non è presente nel menu, ovvero: " + tipo);
        };
